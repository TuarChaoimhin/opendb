...
class tmdb extends SitePlugin {
    ...
    function queryListing($page_no, $items_per_page, $offset, $s_item_type, $search_vars_r) {
        ...
        if (strlen($search_vars_r['tmdb_id']) > 0) {
            ...
            $jsonDataMovies = json_decode($this->fetchURI($this->baseURL . 'search/movie?api_key=' . $this->apikey . $release . '&query=' . rawurlencode(strtolower($search_vars_r['title']))), true);
            $jsonDataTV = json_decode($this->fetchURI($this->baseURL . 'search/tv?api_key=' . $this->apikey . $release . '&query=' . rawurlencode(strtolower($search_vars_r['title']))), true);
            
            if (!is_null($jsonDataMovies)) {
                foreach ($jsonDataMovies['results'] as $result) {
                    if ($result['poster_path'] == 'null') $result['poster_path'] = file_cache_get_noimage_r('listing');
                    $this->addListingRow($result['original_title'], $this->imgBase . $this->imgResultsSize . $result['poster_path'], 'Movie '.$result['release_date'], array('tmdb_id' => $result['id']));
                }
            }

            if (!is_null($jsonDataTV)) {
                foreach ($jsonDataTV['results'] as $result) {
                    if ($result['poster_path'] == 'null') $result['poster_path'] = file_cache_get_noimage_r('listing');
                    $this->_http_vars['op'] = "site-search";
                    $this->addListingRow($result['original_name'], $this->imgBase . $this->imgResultsSize . $result['poster_path'], 'TV Show '.$result['first_air_date'], array('tmdb_id' => $result['id']));
                }
            }

            if (!is_null($jsonDataMovies) || !is_null($jsonDataTV)) {
                return true;
            } else {
                return false;
            }
     ...
     * - TV Programmes -
     * name             = Programme name.
     * original_name    = Original programme name.
     * 
     */
    function queryItem($search_attributes_r, $s_item_type) {
        ...
        //$jsonDataTV = json_decode($this->fetchURI($this->baseURL . 'tv/' . $search_attributes_r['tmdb_id'] . '?api_key=' . $this->apikey . '&append_to_response=casts', true), true);
